...
export function availabilitySlotKey(
  dayIndex: number,
...
export function parseAvailabilitySlotKey(
  key: AvailabilitySlotKey,
): { dayIndex: number; timeSlotIndex: number } | null {
  ...
  if (
    dayIndex >= AVAILABILITY_DAYS.length ||
...
export function availabilitySlotLabel(key: AvailabilitySlotKey): string {
  const parsed = parseAvailabilitySlotKey(key);
  if (!parsed) {
    return key;
  }
  const day = AVAILABILITY_DAYS[parsed.dayIndex];
  const slot = AVAILABILITY_TIME_SLOTS[parsed.timeSlotIndex];
  return `${day.shortName} ${slot.rangeLabel}`;
}